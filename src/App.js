...
export class App extends Component {
  constructor(props) {
    ...
    this.toggleCurrencySwitcherHide =
      this.toggleCurrencySwitcherHide.bind(this);
  ...
  attributeChangeFromCart(itemId, selected, item, id, value) {
    ...
    this.setState((prevState) => {
      let updatedCartItems = prevState.addItem.map((element) => {
        const isSame = element.selectedAttributes.find((attr) =>
          selected.find(
            (selectedAttr) =>
  ...
  handleSelectedAttributes(id, value, itemId) {
    this.setState((prevState) => ({
      selectedAttributes: prevState.selectedAttributes.some(
        (thingy) => thingy.id === id
      )
    ...
    this.setState((prevState) => {
      let updatedCartItems = prevState.addItem.map((element) => {
        ...
        if (
          ...
          element.selectedAttributes.length === 0
        ) {
          return {
            item,
            itemId,
  ...
  toggleCurrencySwitcherHide() {
    this.setState({
      isCurrencySwitcherHidden: !this.state.isCurrencySwitcherHidden,
    });
  }
    ...
    this.setState((prevState) => {
      const foundIndex = prevState.addItem.findIndex((item) => {
        ...
        const sortedAttributes =
          item.selectedAttributes.sort(attributeSortFunc);
  ...
  componentDidUpdate() {
    this.state.addItem.forEach((element) => {
      if (element.counter === 0) {
        this.setState({
          addItem: this.state.addItem.filter(
